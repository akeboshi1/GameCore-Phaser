...
export class PlayerInfo {
    ...
    public setWalkableArea(value: string): void {
        this.walkableArea = new DrawArea(value, 0x00FF00);
    }

    public setCollisionArea(value: string): void {
        this.collisionArea = new DrawArea(value, 0xFF0000);
    }
}
    ...
    public draw(hTileWidth: number, hTileHeight: number): void {
        ...
        for (let i = 0; i < this.cols; i++) {
            for (let j = 0; j < this.rows; j++) {
                this._graphics.beginFill(this.areaArr[i][j] === 1 ? this._color : 0x0000ff, this.areaArr[i][j] === 1 ? 0.6 : 0.2);
