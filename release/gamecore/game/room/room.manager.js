...
export class RoomManager extends PacketHandler {
  constructor(game) {
    super();
    __publicField(this, "mGame");
    __publicField(this, "mRooms", []);
    __publicField(this, "mCurRoom");
  ...
  hasRoom(id) {
    const idx = this.mRooms.findIndex((room, index) => id === room.id);
    return idx >= 0;
  }
  leaveRoom(room) {
  ...
  onEnterRoom(scene) {
    ...
    let boo = false;
    this.mRooms.forEach((room2) => {
      if (room2 && room2.id === id) {
        boo = true;
        return;
      }
    });
    ...
    return __async(this, null, function* () {
      ...
      if (curRoom) {
        if (curRoom.id === vw.scene.id)
          return;
        yield roomManager.leaveRoom(curRoom);
      }
      if (roomManager.hasRoom(vw.scene.id)) {
        roomManager.onEnterRoom(scene);
      } else {
        this.loadSceneConfig(vw.scene.id.toString()).then((config) => __async(this, null, function* () {
  ...
  loadSceneConfig(sceneID) {
    ...
    if (result === void 0) {
      ...
      return result.then((req) => {
        ...
        return new Promise((resolve, reject) => {
          render.showAlert("\u914D\u7F6E\u52A0\u8F7D\u9519\u8BEF\uFF0C\u8BF7\u91CD\u65B0\u767B\u9646" + reason, true, false).then(() => {
            if (!this.mGame.debugReconnect)
              return;
            render.hidden();
          });
          reject();
  ...
  onEnterResult(packet) {
    const content = packet.content;
    if (!content.result) {
      return;
    }
    const tips = [void 0, "commontips.room_full", "commontips.room_need_password", "commontips.room_password_failure", "commontips.room_dose_not_exists"];
