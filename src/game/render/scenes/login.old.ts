...
export class LoginScene extends BasicScene {
    ...
    private requestGetPhoneCode() {
        const login = this;
        this.mWorld.httpService.requestPhoneCode(login.mNameInputTxt.text, "86").then((response: any) => {
    ...
    private loginByPhoneCode() {
        ...
        this.mWorld.httpService
            .loginByPhoneCode(login.mNameInputTxt.text, login.mVerificationCodeTxt.text, "86")
            .then((response: any) => {
                if (response.code === 200 || response.code === 201) {
