...
export class RoomManager extends PacketHandler implements IRoomManager {
  ...
  private initScene() {
    if (this.connection) {
      const pkt = new PBpacket(op_virtual_world.OPCODE._OP_CLIENT_REQ_VIRTUAL_WORLD_SCENE_CREATED);
    ...
    */
  private start(content: op_client.IOP_VIRTUAL_WORLD_RES_CLIENT_ENTER_SCENE, cb: () => void) {
    if (this.mWorld.game) {
      this.mWorld.game.scene.start(LoadingScene.name, {
        callBack: () => {
          const scene: Phaser.Scene = this.mWorld.game.scene.getScene(PlayScene.name);
