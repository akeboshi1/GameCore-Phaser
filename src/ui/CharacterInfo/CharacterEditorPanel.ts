...
export class CharacterEditorPanel extends Phaser.GameObjects.Container {
    ...
    constructor(scene: Phaser.Scene, x: number, y: number, width: number, height: number, key: string, dpr: number) {
        ...
        this.title = this.scene.make.text({ x: 0, y: -height * 0.5 + 50 * dpr, text: "编辑资料", style: { font: mfont, blod: true, color: "#ffffff", fontFamily: Font.DEFULT_FONT } }).setOrigin(0.5);
        const bg = this.scene.make.graphics(undefined, false);
        bg.clear();
        bg.fillStyle(0xffffff, 0.5);
        bg.fillRect(0, 0, 247 * dpr, 310 * dpr);
        bg.setPosition(-width * 0.5, -189 * dpr);
        const posx = -width * 0.5 + 9 * dpr;
        const posy = -174 * dpr;
        const offsetY = 50 * dpr;
        const nameText = this.scene.make.text({ x: posx, y: posy, text: "用户名", style: { blod: true, color: "#0062BB", fontSize: 15 * dpr, fontFamily: Font.DEFULT_FONT } }).setOrigin(0);
        this.inputText = new InputText(this.scene, posx + 60 * dpr, posy, 162 * dpr, 27 * dpr, {
            type: "input",
            fontSize: 18 * dpr + "px",
            color: "#0062BB",
            align: "center",
        ...
        const line = this.createLine(-230 * this.dpr * 0.5, posy + 35 * this.dpr);
        this.add(bg);
        ...
        const signText = this.scene.make.text({ x: posx, y: posy + offsetY * 5, text: "个性签名", style: { blod: true, color: "#0062BB", fontSize: 15 * dpr, fontFamily: Font.DEFULT_FONT } }).setOrigin(0);
        this.signInput = new InputText(this.scene, posx + 60 * dpr, posy + offsetY * 5, 162 * dpr, 76 * dpr, {
            type: "input",
            fontSize: 18 * dpr + "px",
            color: "#0062BB",
            align: "center",
        ...
        const saveBtn = new NinePatchButton(this.scene, 0, height * 0.5 - 60 * dpr, 112 * this.dpr, 40 * this.dpr, common_key, "yellow_btn", i18n.t("player_info.add_friend"), {
            left: 12 * this.dpr,
            top: 12 * this.dpr,
            right: 12 * this.dpr,
            bottom: 12 * this.dpr
        ...
        backBtn.setPosition(posx + 28 * dpr, -height * 0.5 + 50 * dpr);
