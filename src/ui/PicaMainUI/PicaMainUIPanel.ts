...
export class PicaMainUIPanel extends BasePanel {
    ...
    init() {
        ...
        const ninePatch = new NinePatch(this.scene, 60 * this.dpr / 2, this.mStrengthValue.height / 2 - frame.height - 1 * this.dpr, 62 * this.dpr, frame.height, this.key, "strength_progress", 3, 3, {
            left: 8 * this.dpr,
            top: 3 * this.dpr,
...
class ExpProgress extends Phaser.GameObjects.Container {
    ...
    constructor(scene: Phaser.Scene, key: string, dpr: number, scale: number, world: WorldService) {
        ...
        const bg = new NinePatch(this.scene, progressW / 2, progressH / 2, progressW, progressH, key, "exp_bg", 3, 3, {
            left: 8 * dpr,
            top: 3 * dpr,
        ...
        const progres = new NinePatch(this.scene, progressW / 2, progressH / 2, width, frame.height, key, "exp_progress", 3, 3, {
            left: 8 * dpr,
            top: 3 * dpr,
