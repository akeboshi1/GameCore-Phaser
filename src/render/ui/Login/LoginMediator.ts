...
export class LoginMediator extends BaseMediator {
    ...
    show() {
        if (!this.mView) {
            this.mView = new LoginPanel(this.layerManager.scene, this.render);
        }
        this.mView.show();
    ...
    private onShowVerified() {
        if (!this.verifiedPanel) {
            this.verifiedPanel = new VerifiedPanel(this.layerManager.scene, this.render);
        }
