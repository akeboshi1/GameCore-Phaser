...
export class PicaBusinessMyStreetPanel extends Phaser.GameObjects.Container {
    ...
    public setMyStoreData(content: any) {// op_client.OP_VIRTUAL_WORLD_RES_CLIENT_PKT_MY_STORE
    ...
    protected create() {
        ...
        const storeTitle = this.scene.make.text({ x: storex, y: storebg.y, text: i18n.t("business_street.my_store"), style: { fontSize: 13 * this.dpr, fontFamily: Font.DEFULT_FONT, color: "#FFC51A" } }).setOrigin(0, 0.5);
        ...
        this.storeCountText = this.scene.make.text({ x: btnX + this.newStoreBtn.width * 0.5 + 20 * this.dpr, y: posy, text: "", style: { fontSize: 13 * this.dpr, fontFamily: Font.DEFULT_FONT, color: "#FFC51A" } }).setOrigin(1, 0);
        ...
        this.add(talkAllBtn);
        talkAllBtn.setTextStyle({ fontSize: 15 * this.dpr, fontFamily: Font.DEFULT_FONT, color: "#ffffff" });
        ...
        goOutBtn.setTextStyle({ fontSize: 15 * this.dpr, fontFamily: Font.DEFULT_FONT, color: "#996600" });
    }
...
class MyStoreItem extends Phaser.GameObjects.Container {
    public storeData: any;// op_client.EditModeRoom
    private key: string;
    private dpr: number;
    ...
    constructor(scene: Phaser.Scene, x: number, y: number, width: number, height: number, key: string, dpr: number, zoom: number) {
        ...
        enterBtn.setTextStyle({ fontSize: 10 * dpr, fontFamily: Font.DEFULT_FONT, color: "#996600" });
        enterBtn.x = -posx - enterBtn.width * 0.5 - 10 * dpr;
        enterBtn.on(String(ClickEvent.Tap), this.onEnterHandler, this);
        this.add(enterBtn);
    }

    public setStoreData(data: any) {// op_client.EditModeRoom
