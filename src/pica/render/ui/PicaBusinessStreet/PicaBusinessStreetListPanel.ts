...
export class PicaBusinessStreetListPanel extends Phaser.GameObjects.Container {
    ...
    public setIndustryModels(content: any) {// op_client.OP_VIRTUAL_WORLD_RES_CLIENT_PKT_INDUSTRY_MODELS
        ...
        }, new Handler(this, (tabs: TabButton[]) => {
    ...
    public setStreetListData(datas: any[]) {// op_client.IEditModeRoom
    ...
    protected create() {
        ...
        const storeTitle = this.scene.make.text({ x: storex, y: storebg.y, text: i18n.t("business_street.street"), style: { fontSize: 13 * this.dpr, fontFamily: Font.DEFULT_FONT, color: "#FFC51A" } }).setOrigin(0, 0.5);
        ...
        this.secondaryPanel.setButtonChangeHandler(new Handler(this, (button: TabButton) => {
            button.background.visible = false;

        }), new Handler(this, (button: TabButton) => {
    ...
    private createGrideTable(x: number, y: number, width: number, height: number, capW: number, capH: number) {
        ...
        //     tween.tween();
        // });
        grid.on("cellDown", (cell) => {
            const tween = new TweenCompent(this.scene, cell, { scale: 0.96, pingpang: true });
            tween.tween();
        });
        grid.layout();
        this.add(grid);
...
class PicStreetItem extends Phaser.GameObjects.Container {
    public storeData: any;// op_client.IEditModeRoom
    private key: string;
    private key2: string;
    private dpr: number;
    ...
    constructor(scene: Phaser.Scene, x: number, y: number, width: number, height: number, key: string, key2: string, dpr: number, zoom: number) {
        ...
        this.praiseIcon.texture.setFilter(Phaser.Textures.FilterMode.LINEAR);
    ...
    public setStoreData(data: any, categoriesType: CategoryType) {// op_client.IEditModeRoom
