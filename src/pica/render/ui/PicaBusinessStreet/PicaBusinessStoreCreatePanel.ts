...
export class PicaBusinessStoreCreatePanel extends Phaser.GameObjects.Container {
    ...
    private curSelectData: any;// op_pkt_def.IPKT_INDUSTRY | op_pkt_def.PKT_ROOM_MODEL
    ...
    public setTypeData(datas: any) {// op_pkt_def.IPKT_INDUSTRY[] | op_pkt_def.IPKT_INDUSTRY
    ...
    public setTypeInfo(data: any) {// op_pkt_def.IPKT_INDUSTRY | op_pkt_def.PKT_ROOM_MODEL
    ...
    protected create() {
        ...
        if (this.isFirst) {
            this.recommendedText = this.scene.make.text({ x: 0, y: 20 * this.dpr, text: "Recommended", style: { fontSize: 13 * this.dpr, fontFamily: Font.DEFULT_FONT, color: "#697AFF" } }).setOrigin(0.5, 0);
        ...
        selectBtn.setTextStyle({ fontSize: 15 * this.dpr, fontFamily: Font.DEFULT_FONT, color: "#996600" });
    }
...
class StoreTypeItem extends Phaser.GameObjects.Container {
    public storeData: any;// op_pkt_def.IPKT_INDUSTRY | op_pkt_def.PKT_ROOM_MODEL
    private key: string;
    private dpr: number;
    ...
    public setStoreData(data: any) {// op_pkt_def.IPKT_INDUSTRY | op_pkt_def.PKT_ROOM_MODEL
        this.storeData = data;
        if ("industryType" in data) {// data instanceof op_pkt_def.PKT_INDUSTRY
