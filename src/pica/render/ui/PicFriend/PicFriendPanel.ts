...
export class PicFriendPanel extends BasePanel {
    ...
    constructor(uiManager: UiManager) {
        ...
        this.mSubContanerMap.set(FriendChannel.Notice, NoticeContainer);
    ...
    public addListen() {
        ...
        this.friendContainer.on(EventType.FETCH_FRIEND, this.onFetchFriendHandler, this);
        this.friendContainer.on(EventType.REQ_FRIEND_ATTRIBUTES, this.onReqFriendAttributesHandler, this);
        this.friendContainer.on(EventType.RENDERER_EVENT, this.onRendererEventHandler, this);
        this.friendContainer.on(EventType.REQ_PLAYER_LIST, this.onReqFriendListHandler, this);
    ...
    public removeListen() {
        ...
        this.friendContainer.off(EventType.FETCH_FRIEND, this.onFetchFriendHandler, this);
        this.friendContainer.off(EventType.REQ_FRIEND_ATTRIBUTES, this.onReqFriendAttributesHandler, this);
        this.friendContainer.off(EventType.RENDERER_EVENT, this.onRendererEventHandler, this);
        this.friendContainer.off(EventType.REQ_PLAYER_LIST, this.onReqFriendListHandler, this);
    ...
    private onShowAddFriendHandler(type: FriendChannel) {
        ...
        this.mShowingSubContainer.on(EventType.RENDERER_EVENT, this.onRendererEventHandler, this);
        this.mShowingSubContainer.on(EventType.REQ_PLAYER_LIST, this.onReqFriendListHandler, this);
        this.mShowingSubContainer.on(EventType.REQ_FRIEND_RELATION, this.onReqFriendRelationHandler, this);
        ...
        if (eventName) this.render.renderEmitter(eventName);
    }
...
class FriendContainer extends Container {
    ...
    constructor(protected uiManager: UiManager, width: number, height: number, protected key: string, protected dpr: number, protected uiScale: number) {
        super(uiManager.scene, 0, 0);
    ...
    protected draw() {
        ...
        this.titleText = this.scene.make.text({ x: 0, y: posY, style: { fontSize: 16 * this.dpr, fontFamily: Font.DEFULT_FONT, color: "#0" } }).setOrigin(0.5, 0);
...
class MainContainer extends FriendContainer {
    ...
    constructor(uiManager: UiManager, width: number, height: number, key: string, dpr: number, uiScale: number) {
        super(uiManager, width, height, key, dpr, uiScale);
    ...
    public register() {
        ...
        this.searchBtn.on(ClickEvent.Tap, this.onSeachHandler, this);
        this.onlineCheckBox.on(ClickEvent.Tap, this.onCheckoutOnlineHandler, this);
        this.addFriendBtn.on(ClickEvent.Tap, this.onShowAddFriendHandler, this);
    }

    public unregister() {
        this.searchInput.off("textchange", this.onTextChangeHandler, this);
        this.searchBtn.off(ClickEvent.Tap, this.onSeachHandler, this);
        this.onlineCheckBox.off(ClickEvent.Tap, this.onCheckoutOnlineHandler, this);
        this.addFriendBtn.off(ClickEvent.Tap, this.onShowAddFriendHandler, this);
    }
    ...
    protected draw() {
        ...
        const boxtitle = this.scene.make.text({ x: 6.67 * this.dpr + this.onlineCheckBox.width * 0.5, y: 0, text: i18n.t("friendlist.olinetitle"), style: { fontSize: 9.33 * this.dpr, fontFamily: Font.DEFULT_FONT, color: "#0098D8" } }).setOrigin(0, 0.5);
        ...
        this.friendTabel = this.createGrideTable(0, this.topContent.y + this.topContent.height + 380 * this.dpr * 0.5 + 18 * this.dpr, 275 * this.dpr, 380 * this.dpr, 275 * this.dpr, 36 * this.dpr, (scene, cell) => {
            const item = new PicFriendItem(this.render.uiManager, 0, 0, this.key, this.dpr);
            item.on(EventType.REQ_FRIEND_ATTRIBUTES, this.onFtechPlayerHandler, this);
            ...
            item.on(EventType.RENDERER_EVENT, this.onRendererEventHandler, this);
            return item;
    ...
    protected onRendererEventHandler(event: string, ...args) {
        this.emit(EventType.RENDERER_EVENT, event, args);
    }
...
class PicFriendItem extends Container {
    ...
    constructor(uiManager: UiManager, x: number, y: number, key: string, dpr: number) {
    ...
    setItemData(data: FriendData) {
        ...
        switch (menudata.type) {
            case FriendChannel.Notice:
                ...
                this.text.text = i18n.t("friendlist.notice");
                break;
            case FriendChannel.Blacklist:
...
class NoticeRenderer implements IRenderer {
    private icon: Image;
    ...
    constructor(private scene: Phaser.Scene, private owner: PicFriendItem) {
        ...
        this.text = new BBCodeText(this.scene, this.icon.x + this.icon.width + 9.67 * owner.dpr, 0, "", {
            fontSize: 9.33 * owner.dpr,
            fontFamily: Font.DEFULT_FONT,
            color: "#000000"
        });
    ...
    setItemData(data: any) {
        this.text.setText(`[color=#00dffc]${data.nickname}[/color] ${i18n.t("friendlist.followed_you")}`);
    }
...
class FriendRenderer implements IRenderer {
    ...
    protected fans() {
        ...
        this.friendEvnet = EventType.FOLLOW;
    }

    protected followed() {
        this.addBtn.visible = true;
        this.addBtn.setText(i18n.t("friendlist.unfollow"));
        this.friendEvnet = EventType.UNFOLLOW;
    }

    protected blacklist() {
        this.addBtn.visible = true;
        this.addBtn.setText(i18n.t("friendlist.remove"));
        this.friendEvnet = EventType.REMOVE_FROM_BLACKLIST;
    }
    ...
    protected onHeadhandler() {
        this.owner.emit(EventType.REQ_FRIEND_ATTRIBUTES, this.itemData);
    }

    protected onAddHandler() {
        if (this.friendEvnet && this.itemData) this.owner.emit(EventType.RENDERER_EVENT, this.friendEvnet, this.itemData.id);
...
class SubFriendContainer extends FriendContainer {
    ...
    public register() {
        this.backBtn.on(ClickEvent.Tap, this.onBackHandler, this);
    }

    public unregister() {
        this.backBtn.off(ClickEvent.Tap, this.onBackHandler, this);
    }
    ...
    protected draw() {
        ...
        this.gridTable = this.createGrideTable(0, 18 * this.dpr, 275 * this.dpr, 400 * this.dpr, 275 * this.dpr, 36 * this.dpr, () => {
            const item = new PicFriendItem(this.uiManager, 0, 0, this.key, this.dpr);
            item.on(EventType.REQ_FRIEND_ATTRIBUTES, this.onFtechPlayerHandler, this);
            item.on(EventType.RENDERER_EVENT, this.onRendererEventHandler, this);
            return item;
...
class SearchContainer extends SubFriendContainer {
    ...
    private onSearchHandler(text) {
        this.emit(EventType.RENDERER_EVENT, EventType.SEARCH_FRIEND, text);
        this.resize();
    }
}

class BlackContainer extends SubFriendContainer {
    constructor(uiManager: UiManager, width: number, heigth: number, key: string, dpr: number, uiScale: number) {
        super(uiManager, width, heigth, key, dpr, uiScale);
    ...
    fetchEventName() {
        return EventType.REQ_BLACKLIST;
    }
}
...
class SearchInput extends LabelInput {
    private searchBtn: Button;
    private render: Render;
    constructor(uiManager: UiManager, config: any, key: string, dpr: number) {
        super(uiManager.scene, config);
        this.render = uiManager.render;
        ...
        const background = uiManager.scene.make.graphics(undefined, false);
...
class NoticeContainer extends SubFriendContainer {
    constructor(uiManager: UiManager, width: number, heigth: number, key: string, dpr: number, uiScale: number) {
        super(uiManager, width, heigth, key, dpr, uiScale);
        this.friendType = FriendChannel.Search;
    }
    ...
    draw() {
        super.draw();
        this.titleText.setText(i18n.t("friendlist.notice"));
    }
    ...
    setItems(data: any[]) {
        ...
        const ids = [];
        const now = Date.now();
        const showTiems = 72 * 60 * 60 * 1000;
        for (const friend of data) {
            if (friend.created_at) {
                const create_at = Date.parse(friend.created_at);
                if (now - create_at < showTiems) {
                    const user = friend.user;
                    if (user) {
                        result.push({ type: FriendChannel.Notice, id: user._id, nickname: user.nickname });
                        ids.push(user._id);
                    }
                }
            }
