...
export class PicaHandheldPanel extends BasePanel {
    ...
    public setEqipedDatas(content: any) {
        ...
        const emptyData = { id: "empty_handed", count: 0 };
        let curr: any;
...
class HandheldEqiped extends Phaser.GameObjects.Container {
    ...
    constructor(scene: Phaser.Scene, x: number, y: number, key: string, dpr: number) {
        ...
        this.shortcutbg.texture.setFilter(Phaser.Textures.FilterMode.LINEAR);
        this.shortcutIcon = this.scene.make.image({ key, frame: "hand" });
        this.shortcutIcon.texture.setFilter(Phaser.Textures.FilterMode.LINEAR);
