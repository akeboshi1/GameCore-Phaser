...
import { GameGridTable, BBCodeText, NinePatch } from "apowophaserui";
import { Coin, Font, Url } from "utils";
import { DynamicImage } from "gamecoreRender";
export class PicaEquipUpgradeItem extends Phaser.Events.EventEmitter {
    ...
    private create() {
        ...
        this.titleName = this.mScene.make.text({ x: 0, y: this.topbg.y + 0 * this.dpr, text: "矿镐", style: { blod: true, color: "#8F4300", fontSize: 14 * this.dpr, fontFamily: Font.DEFULT_FONT } }).setOrigin(0.5, 0.5);
        this.titleName.setFontStyle("bold");
        this.bottombg = this.mScene.make.image({ x: 0, y: 42 * this.dpr, key: this.key, frame: "bottombg" });
        this.equipName = this.mScene.make.text({ x: -120 * this.dpr, y: 15 * this.dpr, text: "精铁镐", style: { color: "#8F4300", fontSize: 14 * this.dpr, fontFamily: Font.DEFULT_FONT } });
    ...
    private createBtn() {
        ...
        const btnBg = new NinePatch(this.mScene, 0, 0, 88 * this.dpr, 31 * this.dpr, this.commonKey, "yellow_btn_normal", undefined, undefined, {
            left: 12 * this.dpr,
            top: 12 * this.dpr,
            right: 12 * this.dpr,
            bottom: 12 * this.dpr
