...
export class PicaEquipUpgradePanel extends BasePanel {
    private commonkey = "common_key";
    private blackBg: Phaser.GameObjects.Graphics;
    private equipItems: PicaEquipUpgradeItem[] = [];
    ...
    constructor(uiManager: UiManager) {
        ...
        this.key = ModuleName.PICAEQUIPUPGRADE_NAME;
    }
    ...
    init() {
        ...
        this.tilteName = this.scene.make.text({ x: 0, y: posY, text: i18n.t("equipupgrade.title"), style: { color: "#8F4300", fontSize: 15 * this.dpr, fontFamily: Font.DEFULT_FONT } }).setOrigin(0.5, 0);
    ...
    setEquipDatas(content: op_client.OP_VIRTUAL_WORLD_REQ_CLIENT_MINING_MODE_SHOW_SELECT_EQUIPMENT_PANEL) {
        ...
        for (const value of arr) {
            ...
            const item = new PicaEquipUpgradeItem(this.scene, this, this.dpr, this.scale, this.key, this.commonkey);
    ...
    private onReqActiveEquipment(id: string) {
        this.render.renderEmitter(ModuleName.PICAEQUIPUPGRADE_NAME + "_reqActive", id);
    }

    private onReqEquipedEquipment(id: string) {
        this.render.renderEmitter(ModuleName.PICAEQUIPUPGRADE_NAME + "_reqEquiped", id);
    }
}
