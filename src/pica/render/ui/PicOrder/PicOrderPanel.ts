...
// import { ImageValue } from "../components/image.value";
...
//         this.acceleSpend = new ImageValue(scene, 30 * dpr, 10 * dpr, this.key, "iv_coin_s", this.dpr);
...
//         this.calcuTime = new ImageValue(scene, 30 * dpr, 10 * dpr, this.key, "iv_coin_s", dpr);
...
//         // this.text = scene.make.text({ x: 0, y: this.bg.height * 0.5 + 13 * dpr, text: "10", style: { color: "#144B99", fontSize: 10 * dpr, fontFamily: Font.DEFULT_FONT } });
...
//         // item.setFrameValue(`${reward.count}`, this.key, this.getIconName(reward.display.texturePath) + "_s");
//         // this.text.text = `${data.name} x ${data.count}`;
