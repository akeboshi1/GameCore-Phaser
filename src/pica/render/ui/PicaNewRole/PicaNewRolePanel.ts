...
import { BasePanel, ButtonEventDispatcher, DynamicImage, ImageValue, ItemInfoTips, ProgressMaskBar, ThreeSlicePath, UIDragonbonesDisplay, UiManager } from "gamecoreRender";
...
export class PicaNewRolePanel extends BasePanel {
    ...
    public addListen() {
        ...
        this.openBigBtn.on("pointerup", this.OnClosePanel, this);
    }

    public removeListen() {
        if (!this.mInitialized) return;
        this.openBigBtn.off("pointerup", this.OnClosePanel, this);
    }
    ...
    preload() {
        this.addAtlas(this.key, "newrole/newrole.png", "newrole/newrole.json");
    ...
    init() {
        ...
        this.levelLabel = this.scene.make.text({ x: -conWdith * 0.5 + 20 * this.dpr, y: this.bg.y + 7 * this.dpr, text: i18n.t("work.salary") + ":", style: UIHelper.whiteStyle(this.dpr, 11) });
        ...
        this.vipvalue.x = this.levelLabel.x + this.levelLabel.width + this.vipvalue.width * 0.5 + 3 * this.dpr;
        ...
        this.openBigBtn = new Button(this.scene, this.key, "close", "close");
        this.openBigBtn.y = this.bg.y + 2 * this.dpr;
        this.openBigBtn.x = this.content.width * 0.5 - this.openBigBtn.width * 0.5 - 10 * this.dpr;
        ...
        this.content.add([this.headIcon, this.bg, this.openBigBtn, namebg, this.titleName, this.levelLabel, this.vipvalue, this.workbutton, this.starSprite, this.moneySprite, this.openBigBtn, this.itemtips]);
        this.resize();
        super.init();
    ...
    public setJobData(data: op_client.IPKT_Quest) {
        ...
        this.vipvalue.setText(rewardTex);
        const matrr = data.targets;
        for (const item of this.imageValues) {
            item.visible = false;
        }
        let posx = this.vipvalue.x + this.vipvalue.width * 0.5 + 20 * this.dpr;
