...
import { BasePanel, NinePatch, UiManager } from "gamecoreRender";
import { ModuleName, RENDER_PEER } from "structure";
import { Background, Border, Size, Url } from "utils";

export class PicaElementStoragePanel extends BasePanel {
    ...
    private mDragData: any;// op_client.ICountablePackageItem
    private mCloseBtn: Phaser.GameObjects.Image;
    private mDragging: boolean;
    constructor(uiManager: UiManager) {
        super(uiManager.scene, uiManager.render);
        this.key = ModuleName.PICAELEMENTSTORAGE_NAME;
        this.setTween(false);
        this.dpr /= 2;
    }
    ...
    resize(oriention?: number) {
        const size: Size = this.render.getSize();
        this.scale = this.dpr;
    ...
    public setProps(data: any) {// op_client.IOP_VIRTUAL_WORLD_RES_CLIENT_EDIT_MODE_QUERY_EDIT_PACKAGE
    ...
    private setPackageNum(val: number) {
        ...
        this.render.renderEmitter(RENDER_PEER + "_" + this.key + "_queryElement", { page: this.mPacgeNum, perPage: 32 });
    }
    ...
    private onClick(data: any) {// op_client.ICountablePackageItem
    ...
    private onPointerOutHandler() {
        if (this.mDragImage && this.mDragging) {
            this.render.renderEmitter(RENDER_PEER + "_" + this.key + "_selectedElement", this.mDragData);
            this.mDragImage.setTexture(undefined);
            this.mDragging = false;
        }
    }
