...
export class TaskPanel extends BasePanel {
    ...
    init() {
        ...
        this.resize();
        super.init();
        this.render.renderEmitter(ModuleName.PICATASK_NAME + "_questlist");
    }
    ...
    private onFinishHandler(id: string) {
        this.render.renderEmitter(ModuleName.PICATASK_NAME + "_submitquest", id);
    }
    ...
    private onExtendsHandler(isExtend: boolean, item: TaskItem) {
        ...
        if (isExtend) {
            this.render.renderEmitter(ModuleName.PICATASK_NAME + "_questdetail", item.questData.id);
