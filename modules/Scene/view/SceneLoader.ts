...
export class SceneLoader {
  ...
  public changedToMap(
    sceneInfo: SceneInfo,
    freeRes: boolean = true,
    hideLoadProgress: boolean = false
  ): void {
    this.info = sceneInfo;
    ...
    for (let i = 0; i < len; i++) {
      ...
      loader = Globals.LoaderManager.createAtlasLoader(
        key,
        Load.Url.getRes(display.texturePath),
        Load.Url.getRes(display.dataPath)
      );
